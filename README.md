...
| 推送层 | **图片随动**（图文/视频封面/转发原图，最多 9 张，可关）；**转发 / 抽奖类默认不推送**（可配置） |

## 推送规则（v2.1 新增）

- **图片**：`bdw_push_images`（默认开）——图文动态的图、视频封面、转发原动态的图、
  RSS 条目图片（enclosure / &lt;img&gt;）会随推送消息一起发送，最多 9 张；
- **转发动态**：`bdw_ignore_forward`（默认开）——【转发】类型不推送；
- **抽奖/中奖类**：`bdw_ignore_lottery`（默认开）+ `bdw_ignore_keywords`
  （默认：抽奖 / 中奖 / 开奖 / 锦鲤 / 转发抽奖）——正文或转发原动态命中关键词即不推送；
- 被规则忽略的动态**仍会记录已见**，不会反复出现；`bd状态` 可查看当前规则开关。
...
| `bdw_ignore_keywords` | 抽奖类忽略关键词（每行一个） | `["抽奖","中奖","开奖","锦鲤","转发抽奖"]` |
